...
class Solution:
    def mergeTrees(self, root1: Optional[TreeNode], root2: Optional[TreeNode]) -> Optional[TreeNode]:
        if root1 is None and root2 is None: return None
        temp_node = TreeNode(0)
        temp_node.val = (root1.val if root1 else 0)  +  (root2.val if root2 else 0)
        temp_node.left = self.mergeTrees((root1 and root1.left ),(root2 and root2.left))
        temp_node.right = self.mergeTrees((root1 and root1.right), (root2 and root2.right))
        return temp_node
